function [A,C,K,rhob] = iss_rand(n,m,rhoa,dis,miniphase)
...
% miniphase  - ensure that the model is minimum-phase
%
% A,C,K - innovations form state space parameters
%
% We need rhoa < 1 for a stable model. This routine creates A,K,C as random
% Gaussian matrices and scales A so that its spectral norm is equal to rhoa.
% If the 'miniphase' flag is set to true (default), then C and K are each
% scaled by a factor sqrt(r), so that the AR transition matrix A - r*K*C has
% spectral norm < 1, thereby ensuring minimum phase; the subroutine 'speclim'
% is used to identify minimum and maximum r between which minimum-phase holds,
% and r is chosen uniformly from this range.

assert(rhoa < 1);

if nargin < 4 || isempty(dis),       dis       = false; end
if nargin < 5 || isempty(miniphase), miniphase = true;  end
